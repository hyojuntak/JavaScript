...
//입력한값이 currentUser이 아니면 
//display : block 기능이 화면에 출력되면서 
//form의 eventListener함수를 통해 html의 요소인 submit기능을 활성화시켜준다.
//그리고 아래에 만들어진 함수인 handleSubmit을 인자로 받아들여 addEventListner함수에 넣어서 
//동작하게 한다. 
...
//form의input에 값을 입력하면 페이지를 어딘가로 이동하게 되는데 
//현재페이지에선 새로고침이 되며 입력값이 사라지게 되는데 
//이를 막기 위해 preventDefault메소드를 통해 입력값이 사라지지 않게 한다. 
...
//parameter로 텍스트를 받는다. input 태그에 사용자가 입력했을때
//그 입력값을 받아서 localstorage에 저장한다.
